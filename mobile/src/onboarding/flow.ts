 ...
 * flow (Language → Welcome → Account) plus the first-launch gating and the
 * shape that gets persisted. No storage, no navigation, no React here — the
 * app root feeds actions in and reads state out, so the whole flow is
 * unit-testable. The slides themselves are KUR-272 / 273 / 274; storage lives
 * in ./storage.ts.
 */

export type OnboardingStep = 'language' | 'welcome' | 'account';

/** Slide order; also the source of truth for the page-indicator count. */
export const ONBOARDING_STEPS: readonly OnboardingStep[] = ['language', 'welcome', 'account'];
